...
open class MarkdownFile(val path: Path) : Element(path.name) {
    ...
    protected open fun createHtml(src: String, tree: ASTNode, flavour: MarkdownFlavourDescriptor) = HtmlGenerator(src, tree, flavour).generateHtml()
    override fun _generate(location: Path) {
        val src = preprocessMarkdown(path.readText())
        val flavour = CommonMarkFlavourDescriptor()
        val parsedTree = MarkdownParser(flavour).buildMarkdownTreeFromString(src)
        location.resolve(path.nameWithoutExtension + ".html").writeText(createHtml(src, parsedTree, flavour))
    }
